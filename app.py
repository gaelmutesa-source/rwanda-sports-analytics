...
def calculate_analytics(df):
    ...
    # Ensure we only calculate medians for numeric columns to avoid errors
    ...
    df['TPI'] = (df['Tech_Score'] * weights['Technical'] + 
                 ...
                 df['Mental'] if 'Mental' in df else df['Ment_Score'] * weights['Mental'])
...
if uploaded_file:
    ...
    # CRITICAL: TABS MUST BE DEFINED HERE
    ...
    with tab1:
        ...
        with col_ctrl1:
            p1_name = st.selectbox("Select Primary Player", df['player_name'].unique(), key="p1_main")
        with col_ctrl2:
            compare_mode = st.checkbox("Enable Comparison Mode", key="comp_mode")
        ...
        p2_data = None
        if compare_mode:
            p2_name = st.selectbox("Select Comparison Player", df['player_name'].unique(), index=1, key="p2_comp")
    ...
    with tab2:
        ...
        fig_scatter.add_trace(go.Scatter(x=df['Phys_Score'], y=df['TPI'], mode='markers+text', text=df['player_name'], textposition="top center", marker=dict(size=12, color=df['TPI'], colorscale='Viridis', showscale=True)))
        fig_scatter.update_layout(title="Efficiency vs. Impact Map", xaxis_title="Physical Readiness", yaxis_title="Total Performance Index")
        st.plotly_chart(fig_scatter, use_container_width=True)
        
        # Depth & Fatigue
        col_s1, col_s2 = st.columns(2)
        with col_s1:
            st.subheader("Squad Depth")
        ...
        with col_s2:
            ...
            else: st.success("All players are in peak physical condition.")
